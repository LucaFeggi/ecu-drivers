...
inline constexpr capability mdio_support{implementation_status::available,
                                         "direct IEEE 802.3 Clause 22 MDIO"};
inline constexpr capability mac_support{
    implementation_status::available,
    "direct STM32H5 Ethernet MAC/DMA descriptor rings"};

class error {
public:
  explicit constexpr error(hal::ethernet::error_kind kind) noexcept
      : kind_{kind} {}
...
public:
  ...
  [[nodiscard]] auto read_clause22(std::uint8_t phy, std::uint8_t reg)
    ...
    registers_.MACMDIOAR =
        mdio_clock_code() | (static_cast<std::uint32_t>(phy) << 21U) |
        (static_cast<std::uint32_t>(reg) << 16U) | mdio_read | mdio_busy;
    if (!wait_idle()) {
      return result<std::uint16_t, error_type>::failure(
          error{hal::ethernet::error_kind::io});
    }
  ...
  [[nodiscard]] auto write_clause22(std::uint8_t phy, std::uint8_t reg,
    ...
    registers_.MACMDIOAR =
        mdio_clock_code() | (static_cast<std::uint32_t>(phy) << 21U) |
        (static_cast<std::uint32_t>(reg) << 16U) | mdio_write | mdio_busy;
...
private:
  ...
  [[nodiscard]] std::uint32_t mdio_clock_code() const noexcept {
    ...
    constexpr std::uint32_t divisors[] = {42U, 62U, 16U, 26U, 102U, 124U, 4U,
    ...
    for (std::size_t index = 0U;
         index < sizeof(divisors) / sizeof(divisors[0U]); ++index) {
...
struct unknown_phy_status_decoder {
  template <class Mdio>
  [[nodiscard]] static hal::ethernet::link_state decode(Mdio &, std::uint8_t,
                                                        bool up) noexcept {
...
public:
  ...
  [[nodiscard]] auto link() -> result<hal::ethernet::link_state, error_type> {
    ...
    if (!first) {
      return result<hal::ethernet::link_state, error_type>::failure(
          first.error());
    }
  ...
  [[nodiscard]] auto restart_autonegotiation() -> result<void, error_type> {
    ...
    return mdio_.write_clause22(
        Address, bmcr,
        static_cast<std::uint16_t>(control.value() | (1U << 9U) | (1U << 12U)));
  }
...
public:
  ...
  [[nodiscard]] auto initialize() -> result<void, error_type> {
    if (!configuration_.timeout.valid() ||
        configuration_.maximum_frame_size > BufferSize ||
        BufferSize > 0x3FFFU || (BufferSize % 4U) != 0U ||
        !valid_burst(configuration_.tx_dma_burst) ||
    ...
    registers_.DMACTCR =
        dma_burst_length(configuration_.tx_dma_burst) | transmit_start;
  ...
  [[nodiscard]] auto try_receive(hal::span<std::byte> output)
    ...
    const std::size_t frame_size =
        static_cast<std::size_t>(status & rx_packet_length_mask);
...
private:
  ...
  void recycle_rx(descriptor &entry) noexcept {
    ...
    registers_.DMACRDTPR =
        pointer_word(&rx_descriptors_[(rx_index_ + RxCount - 1U) % RxCount]);
  }

  [[nodiscard]] static std::uint32_t
  to_word(std::byte a, std::byte b, std::byte c, std::byte d) noexcept {
    return static_cast<std::uint32_t>(std::to_integer<std::uint8_t>(a)) |
           (static_cast<std::uint32_t>(std::to_integer<std::uint8_t>(b))
            << 8U) |
           (static_cast<std::uint32_t>(std::to_integer<std::uint8_t>(c))
            << 16U) |
           (static_cast<std::uint32_t>(std::to_integer<std::uint8_t>(d))
            << 24U);
  }
