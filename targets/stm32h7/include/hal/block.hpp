...
namespace hal::stm32h7::block {

inline constexpr capability support{
    implementation_status::available,
    "direct SDMMC block protocol with STM32H7 internal IDMA"};
...
public:
  ...
  [[nodiscard]] auto initialize() -> result<void, error_type> {
    ...
    registers_.CLKCR = (configuration_.clock_control & ~clock_width_mask) |
    ...
    const auto interface_condition = command(8U, 0x1AAU, response::short_response);
    ...
    bool ready = false;
    for (std::uint32_t attempts = configuration_.timeout.iterations; attempts > 0U;
         --attempts) {
...
private:
  ...
  [[nodiscard]] bool command(std::uint32_t index, std::uint32_t argument,
    ...
    const std::uint32_t response_bits =
        response_type == response::long_response
            ? command_response_long
            : response_type == response::short_response ? command_response_short : 0U;
  ...
  void clear_status() noexcept {
    registers_.ICR = status_command_crc_fail | status_data_crc_fail |
                     ...
                     status_tx_underrun | status_rx_overrun | status_command_response |
                     status_command_sent | status_data_end | status_idma_error |
                     status_idma_complete;
  }
  ...
  void decode_csd() noexcept {
    ...
    if (version == 1U) {
      ...
      const std::uint64_t bytes =
          (static_cast<std::uint64_t>(c_size) + 1U) *
  ...
  [[nodiscard]] static std::uint32_t bits(const std::uint32_t (&words)[4U],
                                          unsigned high, unsigned low) noexcept {
  ...
  [[nodiscard]] auto data_transfer(std::uint64_t lba, Buffer buffer, bool write)
    ...
    if (block_count > 0xFFFF'FFFFULL || lba >= block_count_ ||
        ...
        size > 0x01FF'FFFFU ||
        (use_dma() && !has_internal_idma() && size / sizeof(std::uint32_t) >
                                               65'535U)) {
      return failure(hal::block::error_kind::out_of_range);
    }
    const std::uint32_t blocks = static_cast<std::uint32_t>(size / 512U);
    const std::uint32_t command_index = write ? (blocks == 1U ? 24U : 25U)
                                              : (blocks == 1U ? 17U : 18U);
    registers_.DTIMER = 0xFFFF'FFFFU;
    registers_.DLEN = static_cast<std::uint32_t>(size);
    registers_.DCTRL = data_block_size_512 | (write ? 0U : data_direction_to_host) |
                       data_enable;
    ...
    if (use_dma()) {
      if constexpr (has_internal_idma()) {
        ...
        for (std::uint32_t remaining = configuration_.timeout.iterations;
          ...
          const std::uint32_t status = registers_.STA;
          if ((status & (status_idma_error | status_data_crc_fail |
                         status_data_timeout | status_rx_overrun |
                         status_tx_underrun)) != 0U) {
  ...
  [[nodiscard]] bool prepare_data_move(Buffer buffer, bool write) noexcept {
    ...
    if constexpr (has_internal_idma()) {
      ...
      registers_.IDMABASE0 = pointer_word(buffer.data());
      if constexpr (requires(Registers &registers) { registers.IDMABASE1; }) {
        registers_.IDMABASE1 = 0U;
      }
  ...
  [[nodiscard]] static std::uint32_t load_word(const std::byte *data) noexcept {
    return static_cast<std::uint32_t>(std::to_integer<std::uint8_t>(data[0U])) |
           (static_cast<std::uint32_t>(std::to_integer<std::uint8_t>(data[1U])) << 8U) |
           (static_cast<std::uint32_t>(std::to_integer<std::uint8_t>(data[2U])) << 16U) |
           (static_cast<std::uint32_t>(std::to_integer<std::uint8_t>(data[3U])) << 24U);
  }

  [[nodiscard]] bool wait_fifo(std::uint32_t ready, std::uint32_t fault) const noexcept {
    for (std::uint32_t remaining = configuration_.timeout.iterations; remaining > 0U;
         --remaining) {
