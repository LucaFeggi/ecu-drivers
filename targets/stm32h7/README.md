...
The package owns reusable STM32H7 peripheral code and exact-device facts. It does not own
PCB pins, oscillators, external transceivers, application policy, or FreeRTOS integration. The
initial exact-device profile is `STM32H723VGT6`, whose device facts are kept under
`include/hal/device/stm32h723vgt6/`.
...
target_link_libraries(firmware PRIVATE hal::stm32h723vgt6)
```
