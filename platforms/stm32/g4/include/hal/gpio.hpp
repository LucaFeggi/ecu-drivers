...
class error {
public:
  explicit constexpr error(hal::gpio::error_kind kind =
                               hal::gpio::error_kind::unavailable) noexcept
      : kind_{kind} {}
...
template <class Port>
[[nodiscard]] inline bool configure_output(
    Port &port, std::uint8_t pin, hal::gpio::level initial,
  ...
  const std::uint32_t shift = static_cast<std::uint32_t>(pin) * 2U;
  const std::uint32_t field = 3U << shift;
  // Set the latch before changing MODER so the pin never exposes an
  // unintended transition during configuration.
...
template <class Port>
[[nodiscard]] inline bool configure_alternate(
    Port &port, std::uint8_t pin, std::uint8_t alternate_function,
    output_type selected_type = output_type::push_pull,
  ...
  const std::uint32_t afr_mask = 0xFU << afr_shift;
  port.OTYPER = (port.OTYPER & ~mask) |
                (static_cast<std::uint32_t>(selected_type) << pin);
  port.OSPEEDR = (port.OSPEEDR & ~field) |
                 (static_cast<std::uint32_t>(selected_speed) << shift);
...
public:
  ...
  [[nodiscard]] auto output_latch()
      -> result<hal::gpio::level, error_type> {
    if (!valid_) {
...
template <class Port, class Syscfg, class Exti> class EdgeInput {
public:
  using error_type = error;

  EdgeInput(Port &port, Syscfg &syscfg, Exti &exti, std::uint8_t pin,
            std::uint8_t port_code) noexcept
      : input_{port, pin}, syscfg_{syscfg}, exti_{exti},
  ...
  [[nodiscard]] auto configure_edge(hal::gpio::edge selected_edge)
    ...
    const std::uint32_t index = pin_ / 4U;
    const std::uint32_t shift = static_cast<std::uint32_t>(pin_ % 4U) * 4U;
    const std::uint32_t field = 0xFU << shift;
    syscfg_.EXTICR[index] =
        (syscfg_.EXTICR[index] & ~field) |
        (static_cast<std::uint32_t>(port_code_) << shift);
  ...
  [[nodiscard]] bool take_event() noexcept {
    const std::uint32_t pending = exti_.PR1 & mask_;
    if (pending == 0U) {
      return false;
    }
    exti_.PR1 = pending;
