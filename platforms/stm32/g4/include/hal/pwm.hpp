...
namespace hal::stm32g4::pwm {

inline constexpr capability support{
    implementation_status::available,
    "direct timer PWM with optional complementary dead time"};
...
public:
  ...
  [[nodiscard]] auto configure(hal::pwm::config configuration)
    ...
    for (std::uint32_t psc = 0U; psc <= 0xFFFFU; ++psc) {
      ...
      const std::uint64_t numerator =
          static_cast<std::uint64_t>(TimerClockHz) *
    ...
    if (!found || !within_error(best_error, configuration.requested_period.value,
  ...
  [[nodiscard]] auto set_pulse_width(hal::nanoseconds pulse)
    ...
    std::uint64_t ticks =
        (pulse.value * TimerClockHz + denominator / 2U) / denominator;
    const std::uint64_t period_ticks = static_cast<std::uint64_t>(registers_.ARR) + 1U;
    ...
    CompareAccessor::get(registers_, Channel) = static_cast<std::uint32_t>(ticks);
  ...
  [[nodiscard]] auto set_dead_time(hal::nanoseconds duration)
    ...
    if constexpr (!Complementary ||
                  !requires(Registers &value) { value.BDTR; }) {
      (void)duration;
...
private:
    ...
    if constexpr (requires(Registers &value) {
      ...
      if constexpr (Channel <= 2U) {
        registers_.CCMR1 = (registers_.CCMR1 &
                            ~(channel_select_mask | mode_mask)) |
                           (6U << mode_shift) | preload;
      } else {
        registers_.CCMR2 = (registers_.CCMR2 &
                            ~(channel_select_mask | mode_mask)) |
                           (6U << mode_shift) | preload;
      }
    }
  ...
  period_for(std::uint32_t psc, std::uint32_t arr) noexcept {
    return {1'000'000'000ULL * (static_cast<std::uint64_t>(psc) + 1U) *
            (static_cast<std::uint64_t>(arr) + 1U) / TimerClockHz};
  }

  [[nodiscard]] static constexpr bool within_error(std::uint64_t error_value,
                                                    std::uint64_t requested,
                                                    std::uint32_t ppm) noexcept {
    return error_value * 1'000'000ULL <= requested * ppm;
  }
