...
public:
  ...
  [[nodiscard]] auto read() -> result<hal::utc_time, error_type> {
    ...
    return result<hal::utc_time, error_type>::success(
        {days_from_civil(year, month, day) * 86'400LL +
             static_cast<std::int64_t>(hour) * 3'600LL +
  ...
  [[nodiscard]] auto set_alarm(hal::utc_time value)
    ...
    for (std::uint32_t remaining = timeout_.iterations; remaining > 0U;
         --remaining) {
      if ((registers_.ICSR & flag_alarm_write) != 0U) break;
      if (remaining == 1U) {
        write_protected(true);
        return failure(hal::rtc::error_kind::clock_failure);
      }
    }
...
private:
  ...
  [[nodiscard]] static bool split(hal::utc_time value, int &year,
                                  ...
                                  unsigned &minute, unsigned &second) noexcept {
    ...
    for (int candidate_year = 2000; candidate_year <= 2099;
         ++candidate_year) {
      for (unsigned candidate_month = 1U; candidate_month <= 12U;
        ...
        if (days >= first &&
            days < first + static_cast<std::int64_t>(length)) {
          year = candidate_year;
  ...
  [[nodiscard]] auto write_calendar(int year, unsigned month, unsigned day,
    ...
    for (std::uint32_t remaining = timeout_.iterations; remaining > 0U;
      ...
      if ((registers_.ICSR & flag_initf) != 0U) {
        registers_.TR = binary_to_bcd(second) |
                        (binary_to_bcd(minute) << 8U) |
                        (binary_to_bcd(hour) << 16U);
        registers_.DR = binary_to_bcd(day) | (binary_to_bcd(month) << 8U) |
                        (binary_to_bcd(static_cast<unsigned>(year - 2000))
                         << 16U);
        registers_.ICSR = registers_.ICSR & ~flag_init;
        write_protected(true);
        return result<void, error_type>::success();
  ...
  [[nodiscard]] auto failure(hal::rtc::error_kind kind)
      -> result<void, error_type> {
