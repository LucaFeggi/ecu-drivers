...
namespace hal::stm32g4::watchdog {
...
class Feeder {
  static_assert(LsiFrequencyHz > 0U && Reload <= 0x0FFFU);
...
public:
  ...
  properties() noexcept {
    ...
    return {hal::nanoseconds{0U},
            hal::nanoseconds{(static_cast<std::uint64_t>(Reload) + 1U) *
                             divider * 1'000'000'000ULL / LsiFrequencyHz},
            true};
  }

  [[nodiscard]] auto start() -> result<void, error_type> {
    if (!timeout_.valid() || divider_value() == 0U) {
      return failure(hal::watchdog::error_kind::io);
    }
    ...
    return failure(hal::watchdog::error_kind::hardware_fault);
  }
