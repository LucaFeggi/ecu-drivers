...
public:
  ...
  [[nodiscard]] auto initialize() -> result<void, error_type> {
    ...
    registers_.CCCR = registers_.CCCR & ~(cccr_fd_operation | cccr_bit_rate_switch);
    if (configuration_.fd_enabled) {
      registers_.CCCR = registers_.CCCR | cccr_fd_operation |
                        (configuration_.bit_rate_switch ? cccr_bit_rate_switch
                                                        : 0U);
    }
    registers_.GFC = reject_remote_frames | reject_nonmatching_frames;
    registers_.SIDFC = encode_location(standard_filter_offset()) |
                       (static_cast<std::uint32_t>(configuration_.standard_filter_count)
                        << 16U);
    registers_.XIDFC = encode_location(extended_filter_offset()) |
                       (static_cast<std::uint32_t>(configuration_.extended_filter_count)
                        << 16U);
    registers_.RXF0C = encode_location(rx_fifo_offset()) |
                       (static_cast<std::uint32_t>(configuration_.rx_fifo_elements)
                        << 16U) |
                       0U;
    registers_.TXBC = encode_location(tx_fifo_offset()) |
                      (static_cast<std::uint32_t>(configuration_.tx_fifo_elements)
                       << 24U);
    registers_.RXESC = rx_fifo_element_size_code;
    registers_.TXESC = tx_element_size_code << 0U;
  ...
  [[nodiscard]] auto try_receive()
    ...
    if (const auto fault = status_error(); fault != hal::can::error_kind::other) {
      return result<hal::can::polled_frame<hal::can::classic_frame>, error_type>::failure(
          error{fault});
    }
    if (fifo_fill() == 0U) {
    ...
    if (index >= configuration_.rx_fifo_elements) {
      return result<hal::can::polled_frame<hal::can::classic_frame>, error_type>::failure(
          error{hal::can::error_kind::io});
    }
    volatile std::uint32_t *element = message_ram_ + rx_fifo_offset() +
                                      static_cast<std::size_t>(index) * element_words;
    const auto frame_id = decode_id(element[0U]);
    const std::uint8_t length = dlc_to_length(static_cast<std::uint8_t>((element[1U] >> 16U) & 0xFU));
    std::array<std::byte, 64U> payload{};
    load_payload(element + 2U, payload, length);
    registers_.RXF0A = index;
    if (length > 8U) {
      return result<hal::can::polled_frame<hal::can::classic_frame>, error_type>::failure(
          error{hal::can::error_kind::protocol_error});
    }
    const auto made = hal::can::classic_frame::make(frame_id, {payload.data(), length});
    if (!made) {
      return result<hal::can::polled_frame<hal::can::classic_frame>, error_type>::failure(
          error{hal::can::error_kind::protocol_error});
    }
  ...
  [[nodiscard]] auto try_receive_fd()
    ...
    if (!initialized_ || !configuration_.fd_enabled) {
      return result<hal::can::polled_frame<hal::can::fd_frame>, error_type>::failure(
          error{hal::can::error_kind::configuration});
    }
    if (const auto fault = status_error(); fault != hal::can::error_kind::other) {
      return result<hal::can::polled_frame<hal::can::fd_frame>, error_type>::failure(
    ...
    volatile std::uint32_t *element = message_ram_ + rx_fifo_offset() +
                                      static_cast<std::size_t>(index) * element_words;
    const auto frame_id = decode_id(element[0U]);
    const std::uint8_t dlc = static_cast<std::uint8_t>((element[1U] >> 16U) & 0xFU);
    ...
    return result<hal::can::polled_frame<hal::can::fd_frame>, error_type>::success(
        hal::can::polled_frame<hal::can::fd_frame>::available(std::move(made).value()));
  }

  [[nodiscard]] auto set_filters(hal::span<const hal::can::filter> filters)
      -> result<void, error_type> {
    if (!initialized_ || !filters.valid() ||
        filters.size() > static_cast<std::size_t>(configuration_.standard_filter_count +
    ...
    for (std::size_t index = 0U; index < configuration_.extended_filter_count * 2U;
         ++index) {
    ...
    for (const auto &filter : filters) {
      if (filter.frame_id.extended) {
        volatile std::uint32_t *entry = message_ram_ + extended_filter_offset() +
                                        extended * 2U;
        ...
        volatile std::uint32_t *entry = message_ram_ + standard_filter_offset() +
                                        standard;
        entry[0U] = ((filter.frame_id.value & standard_id_mask) << 16U) |
                    (filter.mask & standard_id_mask) |
                    (2U << 30U) | (1U << 27U);
        ++standard;
      }
    }
...
private:
  ...
  static constexpr std::uint32_t reject_nonmatching_frames{(2U << 2U) |
                                                            (2U << 4U)};
  static constexpr std::uint32_t rx_fifo_element_size_code{7U};
  static constexpr std::uint32_t tx_element_size_code{7U};
  ...
  [[nodiscard]] bool layout_fits() const noexcept {
    if (configuration_.standard_filter_count > 128U ||
        configuration_.extended_filter_count > 64U ||
        configuration_.rx_fifo_elements > 64U ||
        configuration_.tx_fifo_elements > 32U ||
        configuration_.message_ram_offset_words > message_ram_words_ ||
        configuration_.message_ram_offset_words > 0x3FFFU) {
      return false;
    }
    const std::size_t standard_offset = standard_filter_offset();
    const std::size_t extended_offset = extended_filter_offset();
    const std::size_t rx_offset = rx_fifo_offset();
    const std::size_t tx_offset = tx_fifo_offset();
    const std::size_t end = tx_offset +
                            static_cast<std::size_t>(configuration_.tx_fifo_elements) *
                                element_words;
    return standard_offset <= 0x3FFFU && extended_offset <= 0x3FFFU &&
           rx_offset <= 0x3FFFU && tx_offset <= 0x3FFFU && end <= 0x3FFFU &&
           end <= message_ram_words_;
  }

  [[nodiscard]] constexpr std::size_t standard_filter_offset() const noexcept {
    return configuration_.message_ram_offset_words;
  }
  [[nodiscard]] constexpr std::size_t extended_filter_offset() const noexcept {
    return standard_filter_offset() + configuration_.standard_filter_count;
  }
  [[nodiscard]] constexpr std::size_t rx_fifo_offset() const noexcept {
    return extended_filter_offset() +
           static_cast<std::size_t>(configuration_.extended_filter_count) * 2U;
  }
  [[nodiscard]] constexpr std::size_t tx_fifo_offset() const noexcept {
    return rx_fifo_offset() +
           static_cast<std::size_t>(configuration_.rx_fifo_elements) * element_words;
  }
  [[nodiscard]] static constexpr std::uint32_t encode_location(std::size_t words) noexcept {
    return static_cast<std::uint32_t>(words * sizeof(std::uint32_t));
  }

  [[nodiscard]] std::uint32_t fifo_status() const noexcept { return registers_.RXF0S; }
  [[nodiscard]] std::uint32_t fifo_fill() const noexcept { return fifo_status() & 0x7FU; }
  ...
  [[nodiscard]] hal::can::error_kind status_error() noexcept {
    ...
    if ((status & (interrupt_protocol_error | interrupt_error_passive)) != 0U) {
      registers_.IR = status & (interrupt_protocol_error | interrupt_error_passive);
  ...
  [[nodiscard]] auto write_tx(hal::can::id id, hal::span<const std::byte> data,
    ...
    if ((status & tx_fifo_full) != 0U || (status & tx_fifo_free_level_mask) == 0U) {
      return result<bool, error_type>::success(false);
    }
    const std::uint32_t index = (status >> tx_fifo_put_index_position) &
                                tx_fifo_index_mask;
    if (index >= configuration_.tx_fifo_elements) {
      return result<bool, error_type>::failure(
          error{hal::can::error_kind::io});
    }
    volatile std::uint32_t *element = message_ram_ + tx_fifo_offset() +
                                      static_cast<std::size_t>(index) * element_words;
    ...
    for (std::size_t index_byte = 0U; index_byte < data.size(); ++index_byte) {
      ...
      element[2U + word] |= static_cast<std::uint32_t>(
          std::to_integer<std::uint8_t>(data[index_byte])) << shift;
    }
    barrier();
  ...
  [[nodiscard]] static constexpr std::uint8_t dlc_to_length(std::uint8_t dlc) noexcept {
