...
// EXTI is a separate resource from GPIO. The BSP supplies the SYSCFG and
// EXTI instances plus the encoded GPIO port number for the selected line.
...
public:
  ...
  EdgeInput(Port &port, Syscfg &syscfg, Exti &exti, std::uint8_t pin,
      ...
      : input_{port, pin}, syscfg_{syscfg}, exti_{exti},
        mask_{pin < 16U ? std::uint32_t{1U} << pin : 0U},
        pin_{pin}, port_code_{port_code} {
  ...
  [[nodiscard]] auto configure_edge(hal::gpio::edge selected_edge)
    ...
    const std::uint32_t exticr_shift = static_cast<std::uint32_t>(pin_ % 4U) * 4U;
    const std::uint32_t exticr_mask = 0xFU << exticr_shift;
    syscfg_.EXTICR[exticr_index] =
        (syscfg_.EXTICR[exticr_index] & ~exticr_mask) |
