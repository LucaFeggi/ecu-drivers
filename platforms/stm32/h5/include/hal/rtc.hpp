...
template <class Registers, class PersistencePolicy = backup_domain_persistence>
class Clock {
public:
  ...
  [[nodiscard]] auto read() -> result<hal::utc_time, error_type> {
    ...
    if ((registers_.ICSR & flag_rsf) == 0U) {
      registers_.ICSR = registers_.ICSR & ~flag_rsf;
    ...
    const int year =
        2000 + static_cast<int>(bcd_to_binary((date >> 16U) & 0xFFU));
  ...
  [[nodiscard]] auto set_alarm(hal::utc_time value)
    ...
    for (std::uint32_t remaining = timeout_.iterations; remaining > 0U;
         --remaining) {
      if ((registers_.ICSR & flag_alarm_write) != 0U) {
        writable = true;
        break;
      }
    }
    ...
    registers_.ALRMAR = binary_to_bcd(second) | (binary_to_bcd(minute) << 8U) |
                        (binary_to_bcd(hour) << 16U) |
                        (binary_to_bcd(day) << 24U);
    registers_.CR =
        registers_.CR | control_alarm_interrupt | control_alarm_enable;
    write_protected(true);
...
private:
  ...
  static constexpr std::uint32_t flag_alarm{1U};
  ...
  bcd_to_binary(std::uint32_t value) noexcept {
    return static_cast<unsigned>((value & 0x0FU) +
                                 ((value >> 4U) & 0x0FU) * 10U);
  }

  [[nodiscard]] static constexpr std::uint32_t
  binary_to_bcd(unsigned value) noexcept {
  ...
  [[nodiscard]] static constexpr unsigned
  days_in_month(int year, unsigned month) noexcept {
  ...
  days_from_civil(int year, unsigned month, unsigned day) noexcept {
    year -= month <= 2U ? 1 : 0;
  ...
  [[nodiscard]] static bool split(hal::utc_time value, int &year,
  ...
  [[nodiscard]] auto write_calendar(int year, unsigned month, unsigned day,
    ...
    write_protected(false);
    registers_.CR = registers_.CR & ~control_format_12_hour;
    registers_.ICSR = registers_.ICSR & ~flag_init;
    registers_.ICSR = registers_.ICSR | flag_init;
    bool initialized = false;
    for (std::uint32_t remaining = timeout_.iterations; remaining > 0U;
         --remaining) {
      if ((registers_.ICSR & flag_initf) != 0U) {
        initialized = true;
        break;
      }
    }
