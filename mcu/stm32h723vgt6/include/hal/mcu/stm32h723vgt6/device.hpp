#pragma once

#include <cstddef>
#include <cstdint>
#include <string_view>

namespace hal::stm32h7::device::stm32h723vgt6 {

inline constexpr std::string_view name{"STM32H723VGT6"};
inline constexpr std::string_view package{"LQFP100"};
inline constexpr std::size_t package_pin_count{100U};

struct capabilities {
  static constexpr std::uint8_t adc_instances{3U};
  static constexpr std::uint8_t i2c_instances{5U};
  static constexpr std::uint8_t spi_instances{5U};
  static constexpr std::uint8_t fdcan_instances{3U};
  static constexpr bool ethernet_mac{true};
  static constexpr std::uint8_t sdmmc_instances{2U};
  static constexpr bool adc1_adc2_timer_triggered_dma{true};
  static constexpr bool adc3_multi_channel_scan_reliable{false};
};

} // namespace hal::stm32h7::device::stm32h723vgt6
